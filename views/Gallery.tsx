  ...
  return (
    <div className="min-h-screen pt-40 pb-32 px-8 transition-colors duration-500">
      ...
      {galleryItems.length === 0 ? (
        <p className="text-center text-contentDark/50 dark:text-white/40 text-sm">
          Aún no hay materiales en el archivo visual. Agrega imágenes en <span className="font-bold">content/media/library.json</span> o dentro del front‑matter de los posts.
        </p>
      ) : (
        <div className="columns-1 md:columns-2 lg:columns-3 gap-8 space-y-8 max-w-[1600px] mx-auto">
          {galleryItems.map((item) => (
            <div key={item.id} className="relative group overflow-hidden border border-black/5 dark:border-white/5 bg-black/[0.01] dark:bg-white/[0.01]">
              <img
                src={item.image}
                alt={item.caption}
              ...
              <div className="absolute inset-0 bg-white/90 dark:bg-black/90 opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col justify-end p-8 backdrop-blur-sm">
                <span className="text-[9px] font-black tracking-[0.4em] text-neonOrange uppercase mb-2 transition-colors">
                  {item.series || 'Archivo • En tránsito'}
                </span>
                <p className="text-xl font-bold uppercase tracking-tight text-contentDark dark:text-white transition-colors">{item.caption}</p>
