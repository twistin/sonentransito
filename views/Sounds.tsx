...
import { getSoundTracks, getUpcomingProjects } from '../services/contentService';

const soundcloudProfile = 'https://soundcloud.com/sd-carr';
const publishedTracks = getSoundTracks();
const upcomingProjects = getUpcomingProjects();

const renderPlayer = (trackUrl: string, platform: string | undefined, title: string) => {
  if (platform === 'soundcloud') {
    const embedSrc = `https://w.soundcloud.com/player/?url=${encodeURIComponent(trackUrl)}&color=%23ff5f1f&auto_play=false&hide_related=false&show_comments=true&show_user=true&show_reposts=false&show_teaser=true`;
    return (
      <iframe
        title={`Reproducir ${title} en SoundCloud`}
        width="100%"
        height="166"
        scrolling="no"
        frameBorder="no"
      ...
      {publishedTracks.length > 0 ? (
        <section aria-label="Escucha en SoundCloud" className="grid gap-12 mb-32">
          {publishedTracks.map((track) => (
            <article key={track.id} className="group grid md:grid-cols-[1fr_2fr] gap-8 p-8 border border-black/5 dark:border-white/5 bg-black/[0.01] dark:bg-white/[0.01] hover:border-neonPink transition-all duration-500">
              <header className="flex flex-col justify-center">
                <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-neonGreen mb-2">{track.type || 'audio'}</span>
              ...
              <div className="space-y-4">
                <div className="bg-white p-2 border border-black/5 shadow-inner">
                  {renderPlayer(track.url, track.platform, track.title)}
                </div>
              </div>
            </article>
          ))}
        ...
        <p className="text-center text-contentDark/50 dark:text-white/40 mb-32 text-sm">
          Aún no hay lanzamientos publicados. Agrega entradas en <span className="font-bold">content/media/library.json</span> o usa el front‑matter de los posts.
        </p>
      )}
